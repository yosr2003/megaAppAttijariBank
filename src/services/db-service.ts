...
const localDbService = {
  ...
  async deleteP2PProduct(productId: string) { await updateDatabase((db) => { db.p2pProducts = db.p2pProducts.filter((p) => p.id !== productId); db.p2pFavorites = db.p2pFavorites.filter((f) => f.product_id !== productId); }); },
